...
func (h *Handler) GetMuscleVolumeDistribution(c echo.Context) error {
	userID := c.Get("user_id").(string)

	results, err := h.muscleVolumeDistributionUseCase.Get(c.Request().Context(), userID)
	if err != nil {
	...
	return response.SuccessOK(c, http.StatusOK, code.Success, toMuscleVolumeDistributionResponseList(results))
}
