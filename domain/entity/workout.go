...
func (w *Workout) GetCreatedAt() time.Time { return w.CreatedAt }
func (w *Workout) GetUpdatedAt() time.Time { return w.UpdatedAt }

func (w *Workout) AddExercise(exercise WorkoutExercise) error {
	for _, e := range w.Exercises {
		if e.Exercise.ID == exercise.Exercise.ID {
			return domainerr.ErrDuplicateExercise
		}
	}
...
func (w *Workout) UpdateSet(exerciseID, setID string, reps int, weight float64, rpe int) error {
	if reps <= 0 || weight <= 0 {
		return domainerr.ErrInvalidSetValue
	}
	for i, e := range w.Exercises {
		if e.Exercise.ID == exerciseID {
			...
			return domainerr.ErrSetNotFound
		}
	}
	return domainerr.ErrExerciseNotFoundInWorkout
}
...
func (w *Workout) Complete() error {
	if len(w.Exercises) == 0 {
		return domainerr.ErrEmptyWorkout
	}
	for _, e := range w.Exercises {
		if len(e.Sets) == 0 {
			return domainerr.ErrIncompleteWorkout
		}
	}
	w.Completed = true
	w.UpdatedAt = time.Now()
